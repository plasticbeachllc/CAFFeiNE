This document explains how to start the original AFFiNE server (`@affine/server`) locally with Docker.

> **Note**:
>
> The `@affine/server` code is not included in the open-source CAFFEiNE repository due to licensing constraints. Treat this document as historical reference; to run a server you must supply your own compatible backend image or code.
...
Running AFFiNE's server package (`@affine/server`) requires some dev services to be running, i.e.:
...
These steps apply only if you have the proprietary `@affine/server` code checked out locally in `packages/backend/server`. The CAFFEiNE OSS repository does not provide this backend.
